...
def fit_report(inpars, modelpars=None, show_correl=True, min_correl=0.1,
    ...
    if result is not None:
        ...
        add("    chi-square         = %s" % getfloat_attr(result, 'chi2'))
        add("    reduced chi-square = %s" % getfloat_attr(result, 'chi2red'))
...
def fitreport_html_table(result, show_correl=True, min_correl=0.1):
    ...
    stat_row('# variables', result.nfree)
    stat_row('chi-square', gformat(result.chi2))
    stat_row('reduced chi-square', gformat(result.chi2red))
