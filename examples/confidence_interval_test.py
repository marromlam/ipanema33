...
min = iminuit.Minuit(lambda a, b: np.sum((y-f(x, a, b))**2),
                     print_level=-1, pedantic=False, errordef=0.125)
min.migrad()
min.hesse()
...
lim_left = root(lambda p: (
    (y-f(x, p, popt[1]))**2).sum() - np.float64(ss_reach), popt[0]-0.05*0.00034).x[0]
lim_left-popt[0]
lim_right = root(lambda p: (
    (y-f(x, p, popt[1]))**2).sum() - np.float64(ss_reach), popt[0]+0.05*0.00034).x[0]
lim_right-popt[0]

lim_left = root(lambda p: (
    (y-f(x, popt[0], p))**2).sum() - np.float64(ss_reach), popt[1]-0.05*0.00034).x[0]
lim_left-popt[1]
lim_right = root(lambda p: (
    (y-f(x, popt[0], p))**2).sum() - np.float64(ss_reach), popt[1]+0.05*0.00034).x[0]
lim_right-popt[1]
...
print(ipout1.params.dump_latex(
    cols=['value', 'stdev', 'free'], caption='soy la polla'))
        ...
        for j, k2 in enumerate(params):
            ...
            if i <= j:
                for color, x in enumerate(ci[k1].values()):
                    ...
                    axes[j, i].axvline(
                        x=x, color=f'k', linestyle=':', alpha=0.5)
                    _var_lo = unc.ufloat(
...
def romeroopt(params):
    % Finding root
       for l = 1:
            1: maxit
            Jf0 = NumJacobian(f, x0)
            % Jacobian of f at x0
            root = Jf0\(-f(x0)) + x0
            % RELAXED, non relaxed was x0-J ^ -1*f(x0)
            froot = f(root)
...
shit = ipanema.Sample.from_root(
    '/scratch17/marcos.romero/sidecar/2015/MC_Bs2JpsiPhi/v0r5.root')
...
fit_emcee = ipmini.optimize(
    method='emcee', params=ipout1.params, is_weighted=False)
print(fit_emcee)
corner.corner(fit_emcee.flatchain, labels=[f"${p.latex}$" for p in fit_emcee.params.values(
