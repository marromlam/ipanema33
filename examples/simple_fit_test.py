...
p_fit.add({"name": "shift", "value": 0.1, "min": 0,
          "max": 0.5, "latex": r"\delta"})
p_fit.add({"name": "decay", "value": 0.5, "min": 0,
          "max": 1.0, "latex": r"\Gamma"})
p_fit.print()

# Run the fit with BFGS method
result = optimize(
    model, method="bfgs", params=p_fit, args=(data_x,), kwgs={"y": data_y}
)
print(result)
